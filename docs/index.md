...
Macrel main options are listed bellow:

**Command** - First argument, can be either: 
...
**Threads ('-t', '--threads'):** Specify the number of cpus used to process the samples.

**Output ('-o', '--output'):** Folder destination to output

**Output file / Log ('--file-output'):** path to the output file

**Tag ('--tag'):** Generic tag given to all output files

**Fasta file input ('-f', '--fasta'):** path to the input FASTA file. This is used in both the peptides command (where the file is expected to contain short amino-acid sequences) and in the contigs command (where the file is expected to contain longer nucleotide contigs)

**Reads Forward ('-1', '--reads1') and Reverse ('-2', '--reads2') direction:** path to the input FASTQ files.

**Memory ('--mem'):** parameter used during assembly to specify the percent of RAM used (1-0)

**Clustering ('--cluster'):** Whether to pre-cluster the smORFs (at 100%% identity) to avoid repeats

**Force ('--force'):** ignore input errors and continue

**Fasta headers ('--keep-fasta-headers'):** Keep complete FASTA headers [get-smorfs command]

**Temporary directory ('--tmpdir'):** Temporary directory to use (default: $TMPDIR in the environment or /tmp)

**Version ('--version', '-v'):** version of Macrel
