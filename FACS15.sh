...
prodigal="/home/celio/Prodigal/prodigal"
...
cat .callorfinput.fa | parallel -j $j --block $block --recstart '>' --pipe /home/celio/Prodigal/prodigal -c -m -n -p meta -f sco -a callorfs/{#}.pred.smORFs.fa >/dev/null 2>/dev/null
...
then
	...
	awk '{print ">smORF_"NR"\t"$2}' .tmp2 | pigz --best > $outfolder/$outtag.ids.tsv.gz
